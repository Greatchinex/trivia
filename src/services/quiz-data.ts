export const quiz_data: Array<object> = [
  {
    number: 1,
    question: "Capital City of  United Arab Emirates",
    A: "Beirut",
    B: "Abu Dhabi",
    C: "Jamestown",
    D: "Doha",
    answer: "B"
  },
  {
    number: 2,
    question: "The great Victoria Desert is located in",
    A: "Canada",
    B: "West Africa",
    C: "Australia",
    D: "North America",
    answer: "C"
  },
  {
    number: 3,
    question: "Capital City of  Ghana?",
    A: "Accra",
    B: "Bridgetown",
    C: "Castries",
    D: "Jerusalem",
    answer: "A"
  },
  {
    number: 4,
    question:
      "What is part of a database that holds only one type of information",
    A: "Report",
    B: "Field",
    C: "Record",
    D: "File",
    answer: "B"
  },
  {
    number: 5,
    question: "Capital City of  Algeria?",
    A: "Algiers",
    B: "Addis Ababa",
    C: "St. Pierre",
    D: "Yamoussoukro",
    answer: "A"
  },
  {
    number: 6,
    question: "Who is the author of the book 'Nineteen Eighty Four",
    A: "Thomas Hardy",
    B: "Emile Zola",
    C: "George Orwell",
    D: "Walter Scott",
    answer: "C"
  },
  {
    number: 7,
    question:
      "The Mau Mau were terrorists in which country late 50s early 60s?",
    A: "Kenya",
    B: "Sudan",
    C: "Malaysia",
    D: "Ghana",
    answer: "A"
  },
  {
    number: 8,
    question: "What was the name of the atom bomb dropped on Hiroshima?",
    A: "Little Boy",
    B: "Bad Boy",
    C: "Fat Boy",
    D: "Good Boy",
    answer: "A"
  },
  {
    number: 9,
    question: "Which English Monarch succeeded William III of Orange?",
    A: "Mary II",
    B: "James II",
    C: "James I",
    D: "Charles II",
    answer: "A"
  },
  {
    number: 10,
    question:
      "What are the cannibalistic beasts called in H.G. Wells book The Time Machine?",
    A: "Morlocks",
    B: "Warlocks",
    C: "Yahoos",
    D: "Triffids",
    answer: "A"
  },
  {
    number: 11,
    question: "Whose autobiography was entitled 'The Long Walk To Freedom'?",
    A: "Nelson Mandela",
    B: "Mahatma Gandhi",
    C: "Lenin",
    D: "Martin Luther King",
    answer: "A"
  },
  {
    number: 12,
    question: "Who does Mr Brownlow rescue?",
    A: "Oliver Twist",
    B: "Tom Jones",
    C: "Huckleberry Finn",
    D: "Tom Brown",
    answer: "A"
  },
  {
    number: 13,
    question: "In which book is there an inn called Admiral Benbow?",
    A: "Treasure Island",
    B: "Peter Pan",
    C: "Lord of the Rings",
    D: "Gullivers Travels",
    answer: "A"
  },
  {
    number: 14,
    question:
      "In the book The Last Of The Mohicans what was the name of Chingachgook's only son?",
    A: "Uncas",
    B: "Mingo",
    C: "Lightfoot",
    D: "Magua",
    answer: "A"
  },
  {
    number: 15,
    question: "In which Italian city do Romeo and Juliet live?",
    A: "Verona",
    B: "Naples",
    C: "Milano",
    D: "Pisa",
    answer: "A"
  },
  {
    number: 16,
    question: "Who managed the punk group The Sex Pistols?",
    A: "Malcolm McClaren",
    B: "John Lydon",
    C: "Sid Vicious",
    D: "They were unmanagable",
    answer: "A"
  },
  {
    number: 17,
    question: "Barry Manilow had a hit with which song?",
    A: "Mandy",
    B: "Randy",
    C: "Candy",
    D: "Dandy",
    answer: "A"
  },
  {
    number: 18,
    question: "Which ship was a hit for the Beach Boys?",
    A: "Sloop John B.",
    B: "Proud Mary",
    C: "Mississippi Quenn",
    D: "Rainbow Warrior",
    answer: "A"
  },
  {
    number: 19,
    question: "Who appeared a record 106 times on Top Of The Pops?",
    A: "Status Quo",
    B: "Adam and the Ants",
    C: "T-Rex",
    D: "The England World Cup Squad",
    answer: "A"
  },
  {
    number: 20,
    question: "What was a hit for the Pretenders?",
    A: "Stop your sobbing",
    B: "Crying in the rain",
    C: "Tears on my pillow",
    D: "Tears of a clown",
    answer: "A"
  },
  {
    number: 21,
    question: "Which animal was a hit for Jefferson Airplane?",
    A: "White Rabbit",
    B: "White Horses",
    C: "White Dove",
    D: "Ride a white Swan",
    answer: "A"
  },
  {
    number: 22,
    question: "What is Smokey Robinson's real name?",
    A: "William Robinson",
    B: "Joe Robinson",
    C: "Smokey Robinson",
    D: "Samuel Robinson",
    answer: "A"
  },
  {
    number: 23,
    question: "Which band was not a trio?",
    A: "Wings",
    B: "America",
    C: "ELP",
    D: "Cream",
    answer: "A"
  },
  {
    number: 24,
    question: "What lives in a Formicarium?",
    A: "Ants",
    B: "Turtles",
    C: "Apes",
    D: "Butterflies",
    answer: "A"
  },
  {
    number: 25,
    question: 'Who said "nature abhors a vacuum"?',
    A: "Galileo",
    B: "Einstein",
    C: "Aristotle",
    D: "Newton",
    answer: "A"
  },
  {
    number: 26,
    question: "Which of the following animals are not rodents?",
    A: "Hedgehogs",
    B: "Rats",
    C: "Gophers",
    D: "Chipmunks",
    answer: "A"
  },
  {
    number: 27,
    question: "A Blue Whale has a heart roughly the size of a what?",
    A: "VW Beetle",
    B: "Basketball",
    C: "Grapefruit",
    D: "Peanut",
    answer: "A"
  },
  {
    number: 28,
    question: "How many holes are there on a ten pin bowling ball?",
    A: 3,
    B: 4,
    C: 2,
    D: 5,
    answer: "A"
  },
  {
    number: 29,
    question:
      "How many Olympic gold medals did mark Spitz win in Munich in 1972?",
    A: 7,
    B: 8,
    C: 6,
    D: 5,
    answer: "A"
  },
  {
    number: 30,
    question: "Which sport is played at Hickstead?",
    A: "Show Jumping",
    B: "Rally Car Racing",
    C: "Cricket",
    D: "Polo",
    answer: "A"
  },
  {
    number: 31,
    question: "'Wedel' is a term in which sport?",
    A: "Skiing",
    B: "Darts",
    C: "Chess",
    D: "Formula One",
    answer: "A"
  },
  {
    number: 32,
    question: "Figure skating champion Elvis Stojko was from which country?",
    A: "Canada",
    B: "Russia",
    C: "USA",
    D: "Bulgaria",
    answer: "A"
  },
  {
    number: 33,
    question: "Which NHL hockey team has won the most Stanley Cups?",
    A: "Montreal Canadiens",
    B: "Toronto Maple Leafs",
    C: "Boston Bruins",
    D: "New York Rangers",
    answer: "A"
  },
  {
    number: 34,
    question: "A 'Miller' is a move in which sport?",
    A: "Trampolining",
    B: "Diving",
    C: "Chess",
    D: "Ice Hockey",
    answer: "A"
  },
  {
    number: 35,
    question: "Who was dubbed the Elephant Man?",
    A: "John Merrick",
    B: "John McCririck",
    C: "John McEnroe",
    D: "John Martin",
    answer: "A"
  },
  {
    number: 36,
    question: "Duchess is a cat in which film ?",
    A: "Aristocats",
    B: "Lethal Weapon",
    C: "101 Dalmatians",
    D: "Men In Black",
    answer: "A"
  },
  {
    number: 37,
    question:
      "Which of the following was not a character in Disney's Pocahontas?",
    A: "Mooky",
    B: "Flit",
    C: "Chief Powhatan",
    D: "Lon",
    answer: "A"
  },
  {
    number: 38,
    question:
      "Which American President was a famous film/TV star before he became the president ?",
    A: "J.F. Kennedy",
    B: "Geraid R Fond",
    C: "Richard Nixon",
    D: "Ronald Regan",
    answer: "D"
  },
  {
    number: 39,
    question:
      "Joseph Barbera created which one of the following famouse cartoon charecters?",
    A: "Spiderman",
    B: "Mickey Mouse",
    C: "Tom and Jerry",
    D: "Phantom",
    answer: "C"
  },
  {
    number: 40,
    question:
      "In which one of the following films did the Nobel Laureate Pearl S Buck collaborate?",
    A: "Shatranj Ke Khilari",
    B: "Guide",
    C: "Lawrence of Arabia",
    D: "Titanic",
    answer: "B"
  }
];
